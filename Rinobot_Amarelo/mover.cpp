...
void Mover::setAreas(Point2d def, Point2d atk)
{
    if(!againstTheTeam)
    {
        centroidDef = def;
...
{
    Point2d robotPos = teamRobot[indexRobot].getDataState().pos;
    Point2d goalPos = robotFunctions[indexRobot]->getGoal();
    float robotAngle = teamRobot[indexRobot].getDataState().angle;
    float robotVel = sqrt(pow(teamRobot[indexRobot].getDataState().vel.x,2) + pow(teamRobot[indexRobot].getDataState().vel.y,2))/100;
    float robotOmega = teamRobot[indexRobot].getDataState().omega;
    float vMax = this->vMax*0.01;
...
void Mover::kickPenalty()
{
    ...
    if ((ball.pos.y > robotPos.y) && (euclidean_dist(ball.pos, robotPos) < 14) && euclidean_dist(robotPos,Point2d(122,65)) < 10)
    {
        rVel = -velGiroPenalty;
        lVel = velGiroPenalty;
        cout << "Chute Esquerdo" << endl;
    }
    else if ((ball.pos.y < robotPos.y) && (euclidean_dist(ball.pos, robotPos) < distGiro) && euclidean_dist(robotPos,Point2d(122,65)) < 10)
    {
        rVel = velGiroPenalty;
        lVel = -velGiroPenalty;
